module GameServer {
    export class GameMaster {
        ...
        constructor(instance: any) {
            ...
            this.bullets = [];
            this.onconnect = function(data: any) {
                console.log(data);
                this.self = data.id;
            }
        }
        ...
        moveBullet = function(bullet: any) {
            var dt = this.dt;
        ...
        updatePlayers = function(data: any) {
        ...
        clientOnNetMessage = function(data: any) {
        ...
        truncateMessage = function(str: string) {
            var count = 0;
        ...
        clientConnectServer = function(data: any) {
            this.socket = io.connect();
            var player = this.self;
            var sio = this.socket;
                //Now we can listen for that event
            var GM = this;
            sio.on('onconnected', function( data: any ) {
        ...
        clientOnJoinGame = function(data: any) {
        ...
        clientOnHostGame = function(data: any) {
            //this.clientResetPositions();
        ...
        sendToServer = function(key: any, condition: any) {
            this.inputId++;
            var inputId = this.inputId;
            this.socket.send('c.i.'+this.self+'.'+key+'.'+condition+'.'+inputId);
        }
        sendMouseToServer = function(key: any, condition: any, x: number, y: number) {
            this.socket.send('c.m.'+this.self+'.'+key+'.'+condition+'.'+x+'.'+y);
        }
        sendInit = function(color: any, name: any, id: any) {
        ...
        addTiledBackground = function(url: string, x: number, y: number, width: number, height: number) {
        ...
        processPhysics = function(dt: any) {
            this.processPlayerSelfPhysics(dt);
            this.interpolate(dt);
            this.reDraw();
        }

        processPlayerSelfPhysics = function(dt: any) {
            //  console.log('hi');
            var speed = 50;
        ...
        processPlayerPhysics = function(input: any, dt: any) {
            var player = this.findPlayer(this.self);
            //console.log(player);
            if(!player) return;
            var upArrow = 38;
        ...
        processPlayerInput = function(key: any, condition: any) {
            var upArrow = 38;
            var leftArrow = 37;
            var rightArrow = 39;
            var downArrow = 40;
        ...
        updateKeyCondition = function(playerInput: any, condition: any) {
            if(condition === 1) {
