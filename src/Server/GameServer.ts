...
module GameServer { 
    export class GameServer {
        ...
        createGame = function (player: any) {
            var thegame: Game = new Game(UUID(), player, null, 1);
        ...
        findPlayerInput = function (player, key, condition) {
        ...
        clearInput = function (player, key) {
            var inputs = player.atts.inputs;
            for (var i = 0; i < inputs.length; i++) {
                if (inputs[i].key === key) {
                    inputs.splice(i, 1);
        ...
        addClientPlayer = function (player) {
            this.clientState.players.push(player);
        }
        getClientPlayers = function () {
            var returnVal = [];
            var players = this.players;
            for (var i = 0; i < players.length; i++) {
                var player = players[i];
